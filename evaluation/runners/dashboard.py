...
def build_html(
    ...
    if not smrt_tasks:
        smrt_tasks = [{"id": "smrt-paper", "title": "SMRT paper reconstruction"}]
    paper_rows = "".join(
        '<tr><td><strong>%s</strong></td><td>%s</td><td><span class="status pending">Not executed</span></td></tr>'
        % (esc(task.get("id", "")), esc(task.get("title", "SMRT paper task")))
...
<header class="top"><div><div class="eyebrow">PhysEarth-Agent · evaluation</div><h1>Model Registration Tests and Paper Reproduction</h1><p>For first-time users and reviewers: first check whether the registered models are discoverable, valid, and runnable through a small bundled example. Then check whether the selected paper reproduction has actually been completed. The current paper section is limited to SMRT and is explicitly marked as not executed.</p></div><a class="plan-link" href="../../COMPETITION_EVAL_PLAN.html">Read the evaluation plan ↗</a></header>
<nav><a href="#registration">Model registration tests</a><a class="secondary" href="#paper">Paper reproduction results</a></nav>
<div class="hero"><h2>Current evaluation status</h2><p>Registration checks and bundled demos are repeatable local computations. Paper reproduction is the next work item and is not included in the completed result.</p><div class="metrics"><div class="metric"><b>__REGISTERED__</b><span>registered models passed</span></div><div class="metric"><b>__CONTRACT__</b><span>registration checks passed</span></div><div class="metric"><b>__DEMOS__</b><span>bundled demos passed</span></div><div class="metric"><b>Not executed</b><span>SMRT paper reproduction</span></div></div></div>
...
<section id="paper"><div class="section-head"><h2>Paper reproduction results</h2><p>Only SMRT is retained as the paper-reproduction target for now. It has not been executed, so planned work is not presented as completed evidence.</p></div>
<div class="panel"><div class="paper-state"><b>SMRT · Not executed</b><span>No reproduction score, figure comparison, or provenance conclusion is available yet.</span></div><p class="paper-copy">When executed, the Agent must extract the model, parameters, units, and figure target from the paper; separate paper facts from assumptions; run the legal SMRT configuration; and compare it with the paper or an independent reference. The table lists planned cases only.</p><div class="table-wrap"><table class="paper-table"><thead><tr><th>Planned case</th><th>Question</th><th>Status</th></tr></thead><tbody>__PAPER_ROWS__</tbody></table></div><div class="paper-note">The “Not executed” state is intentional. It is not a model failure and does not imply an estimated score. Evidence, source spans, and replay records will be added after the SMRT reproduction run.</div></div></section>
...
def main(argv=None):
    ...
    print(
        "dashboard: %d registered model(s), %d demo(s); SMRT paper reproduction not executed; %d scoring error(s) -> %s"
