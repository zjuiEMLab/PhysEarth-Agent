...
<header class="top"><div><div class="eyebrow">PhysEarth-Agent · evaluation</div><h1>Model Registration Tests and SMRT Scientific Questions</h1><p>For first-time users and reviewers: first check whether the registered models are discoverable, valid, and runnable through a small bundled example. Then inspect four paper-grounded SMRT scientific-question demos. They are bounded pilots and are explicitly marked as not executed.</p></div><a class="plan-link" href="../../../docs/evaluation/COMPETITION_EVAL_PLAN.html">Read the evaluation plan ↗</a></header>
